...
app.get("*", (req, res) => {
    ...
    if (fs.existsSync(indexPath)) {
        ...
        res.status(503).json({
            success: false,
            message: "Client application not built yet. Please run 'npm run build' in the client directory.",
            hint: "From the server directory, run: cd ../client && npm install && npm run build"
        });
    }
});
